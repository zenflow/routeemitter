...
var client = {
    ...
    change: function(mode, route){
        switch (mode){
            case 'push': History.pushState({}, window.document.title, route.url); break;
            case 'replace': History.replaceState({}, window.document.title, route.url); break;
...
function attachLinkHandler(router){
