...
var Route = function(a, b){
    ...
    if (b){
        ...
        self.url = typeof a=='string'?a:(process.browser?client.getDocumentLocation():'');
        self.name = null;
        self.params = null;
...
var RouteEmitter = function(patterns, options){
    ...
    var url = self._bindToWindow && client.getDocumentLocation() || options.url;
    // initialise
    if (self._bindToWindow){client.bindRouter(self);}
    ...
    if (initialEmit){
        process.nextTick(function(){
            ...
            if (self._bindToWindow && (client.getDocumentLocation()!=url)){throw new Error('Document location changed??');}
...
RouteEmitter.prototype.destroy = function(){
    ...
    if (self._bindToWindow){client.unbindRouter(self);}
};
RouteEmitter.prototype.go = function(n){
    ...
    if (!self._bindToWindow){
        throw new Error('RouteEmitter#back(), RouteEmitter#forward(), and RouteEmitter#go() are not implemented ' +
            'for server (yet) and only work on browser with bindToWindow option');
    }
    client.go(n);
};
