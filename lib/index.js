...
var RouteEmitter = function(patterns, options){
    ...
    self._bindToWindow = process.browser && options.bindToWindow;
    var url = self._bindToWindow && getDocumentLocation() || options.url;
    // initialise
    if (self._bindToWindow){routers.push(self);}
    self.Route = self._getRouteClass(self.Route);
    self.last_route = null;
    self.route = self._initial_route = new self.Route(url);
    if (process.browser && self._bindToWindow){self._attachLinkHandler();}
    if (initialEmit){
        process.nextTick(function(){
            ...
            if (self._bindToWindow && (getDocumentLocation()!=url)){throw new Error('Document location changed??');}
...
RouteEmitter.prototype._attachLinkHandler = function(){
    var self = this;
    ...
    self._attachLinkHandler = function(){}; //prevent any further calls to this method for `this` instance by overriding with noop
};
